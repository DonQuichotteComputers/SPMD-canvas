...
${CROSS_PREFIX}gcc src/${PGM}.c -o bin/${PGM}.elf -I ${EINCS} -L ${ELIBS} -le-hal -le-loader

# Build DEVICE side program
# -msmall16 still does not work with 2016.11 ESDK and gcc 5.4
#-mshort-calls still works :D  

#e-gcc -Ofast -T ${ELDF} -msmall16 src/e_${PGM}.c -o bin/e_${PGM}.elf -le-lib
#e-gcc 5.4 makes poor use of the option -mfp-mode=int
#  the option -mfp-iarith slows DOWN my program -- more than 20 % :/ 

 e-gcc -Ofast -T ${ELDF} -mfp-mode=int -mshort-calls -m1reg-r63 src/e_${PGM}.c src/e_${PGM}.S -o bin/e_${PGM}.elf -le-lib
